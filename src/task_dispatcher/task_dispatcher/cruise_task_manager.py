...
from .config import ROBOT_TYPE, PCD_FILE_PATH  # 引入配置文件中的URL和点云文件路径
import subprocess
from auth_utils import AuthManager

# 配置日志
logger = logging.getLogger('task_dispatcher.cruise_task_manager')
...
class CruiseTaskManager:
    """
    巡检任务管理器，负责处理巡检任务相关命令
    ...
    def _processing_cruise_task(self, task_id, task_data):
        ...
        try:
            ...
            if not self.map_manager.check_map_existence(map_file_id):
                logger.info(f'地图 {map_file_id} 不存在，开始更新')
                result = self.map_manager.update_map(map_file_id, map_file_uri)
                if result:
                    self.map_manager.push_map_to_service(map_file_id)
                else:
                    logger.error(f'更新地图 {map_file_id} 失败')
                    self._report_task_status(task_id, TaskStatus.ABORTED.value, f'更新地图 {map_file_id} 失败')
                    return
            else:
                # 地图已存在，直接推送
                self.map_manager.push_map_to_service(map_file_id)
            ...
            if not result.get('success', False):
                self._report_task_status(task_id, TaskStatus.ABORTED.value, '设置机器人站起失败')
                logger.error(f'设置机器人站起失败: {result.get("message", "")}')
                return
            ...
            for cruise in cruises:
                ...
                # 2、处理move动作
                result_action = self.perform_action_manager.perform_action_walk_to_pos(pos)
            ...
            time.sleep(1)
            if not result.get('success', False):
                self._report_task_status(task_id, TaskStatus.ABORTED.value, '停止动作执行失败')
                logger.error(f'停止动作执行失败: {result.get("message", "")}')
            
            # 第五步：巡检任务完成，设置机器人蹲站
            result = self.perform_action_manager.perform_action_stand(argv={"mode": 2})
            if not result.get('success', False):
                self._report_task_status(task_id, TaskStatus.ABORTED.value, '设置机器人蹲站失败')
                logger.error(f'设置机器人蹲站失败: {result.get("message", "")}')
            
            time.sleep(1)
            # 第六步：巡检任务完成，设置机器人趴下
            result = self.perform_action_manager.perform_action_stand(argv={"mode": 0})
            if not result.get('success', False):
                self._report_task_status(task_id, TaskStatus.ABORTED.value, '设置机器人趴下失败')
                logger.error(f'设置机器人趴下失败: {result.get("message", "")}')
            
            logger.info(f"执行巡检任务task_id={task_id}, 任务已完成")
            self._report_task_status(task_id, TaskStatus.COMPLETED.value, '任务已完成', file_id, 'pcd')
            
        except Exception as e:
            logger.error(f'下发巡检任务到机器人时出错: {str(e)}')
            # 上报任务失败状态
    ...
    def _processing_uav_cruise_task(self, task_id, task_data):
        """
        处理无人机巡检任务
        ...
        # 启动点云服务（不检查错误）
        logger.info('启动点云服务...')
        subprocess.run(['sudo', 'systemctl', 'start', 'fast_lio.service'], timeout=10)
        logger.info('启动点云服务操作完成')
    ...
    def _execute_task_thread(self, task_id, body):
        ...
        try:
            if ROBOT_TYPE == "default":
                ...
                self._processing_uav_cruise_task(task_id, body)
            else:
